...
class BaseGETVIEW(DataPermissionMixins, APIView, RewritePageNumberPagination):
    ...
    def get(self, request):
        ...
        try:
            ...
            logger.error(msg="返回状态码:{1},错误信息:{0}".format(
    ...
    def post(self, request):
        ...
        try:
            ...
            if not data.is_valid():
                logger.error(msg="数据验证错误:{0}".format(data.errors))
                raise APIException(
            ...
            logger.error(msg="返回状态码:{1},错误信息:{0}".format(error.default_detail, error.status_code))
            return DataResponse(
                code=error.status_code,
                msg=error.default_detail
            )
...
class UserGETView(DataPermissionMixins, APIView):
    ...
    def get_menu(self):
        ...
        if self.user.is_superuser:
            logger.info(msg="当前为超级用户，用户：{0}!".format(self.user.username))
    ...
    def get(self, request):
        ...
        logger.info(msg="返回:{0}".format(menu))
        return DataResponse(
            data=menu,
            msg="获取信息成功！",
            code='00000'
